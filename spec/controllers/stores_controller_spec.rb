...
RSpec.describe StoresController, type: :controller do
  ...
  describe "GET #index" do
    it "assigns all stores as @stores" do
      get :index
      expect(assigns(:active_stores)).to_not be_nil
      expect(assigns(:inactive_stores)).to_not be_nil
      expect(assigns(:active_stores).map(&:name)).to eq %w[CMU Oakland]
    end
  end
  ...
  describe "GET #show" do
    it "assigns the requested store as @store" do
      ...
      create_assignments
      get :show, id: @cmu
      expect(assigns(:store)).to eq(@cmu)
      expect(assigns(:current_assignments)).to_not be_nil
      expect(assigns(:store).name).to eq "CMU"
      expect(assigns(:current_assignments).map{|a| a.employee.last_name}).to eq %w[Crawford Sisko]
      remove_employees
      remove_assignments
    end
  end
