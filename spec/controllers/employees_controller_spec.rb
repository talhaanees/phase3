...
RSpec.describe EmployeesController, type: :controller do
  ...
  describe "GET #index" do
    it "assigns all employees" do
      get :index
      expect(assigns(:active_employees)).to_not be_nil
      expect(assigns(:inactive_employees)).to_not be_nil
      expect(assigns(:active_employees).map(&:last_name)).to eq(%w[Crawford Gruberman Heimann Janeway Sisko]) 
    end
  end
  ...
  describe "GET #show" do
    it "assigns the requested employee as @employee" do
      ...
      create_assignments
      get :show, id: @ben
      expect(assigns(:employee)).to eq(@ben)
      expect(assigns(:employee)).to_not be_nil
      expect(assigns(:assignments)).to_not be_nil
      expect(assigns(:employee).last_name).to eq "Sisko"
      expect(assigns(:assignments).map(&:pay_level).sort).to eq [3, 4]
      remove_stores
      remove_assignments
    end
  end
